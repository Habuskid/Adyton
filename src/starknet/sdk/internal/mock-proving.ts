...
export class CallMockProofProvider implements ProofProviderInterface {
  ...
  async prove(invocation: ProofInvocation, blockIdentifier?: BlockIdentifier): Promise<Proof> {
    ...
    if (blockIdentifier != null) {
      const block: any = await this.node.getBlock(blockIdentifier);
      baseBlockNumber = BigInt(block.block_number ?? 1);
    } else {
      const latestBlock: any = await this.node.getBlock("latest");
      const currentBlockNumber = BigInt(latestBlock.block_number ?? 1);
      const blocksBack = 10n;
      baseBlockNumber = currentBlockNumber > blocksBack ? currentBlockNumber - blocksBack : 1n;
    }
    const baseBlock: any = await this.node.getBlock(Number(baseBlockNumber));
